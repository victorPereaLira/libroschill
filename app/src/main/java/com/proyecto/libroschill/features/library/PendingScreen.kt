...
fun PendingScreen(
    ...
    val pendingBooks = searchViewModel.books
    ...
    val allGenres = remember(pendingBooks) {
        pendingBooks.flatMap { it.volumeInfo.categories ?: emptyList() }.distinct()
    }

    val filteredBooks = if (selectedGenre.value != null) {
        pendingBooks.filter { it.volumeInfo.categories?.contains(selectedGenre.value) == true }
    } else {
        pendingBooks
    }
    ...
    ScreenWithDrawer(
        ...
        Column(
            ...
            if (filteredBooks.isEmpty()) {
                Text(
                    text = stringResource(R.string.noBooksPending),

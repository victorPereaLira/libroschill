...
fun ReadScreen(
    ...
    val readBooks = searchViewModel.books
    val selectedGenre = remember { mutableStateOf<String?>(null) }

    val allGenres = remember(readBooks) {
        readBooks.flatMap { it.volumeInfo.categories ?: emptyList() }.distinct()
    }

    val filteredBooks = if (selectedGenre.value != null) {
        readBooks.filter { it.volumeInfo.categories?.contains(selectedGenre.value) == true }
    } else {
        readBooks
    }
    ...
    ScreenWithDrawer(
        ...
        Column(
            ...
            // Filtro reutilizable por género
            GenreFilter(
                genres = allGenres,
            ...
            if (filteredBooks.isEmpty()) {
                Text(
                    text = stringResource(R.string.noBooksRead),
